...
<div class="col-sm-4 mb-5">
  ...
  <?php 
 $sql = "SELECT request_id, request_info, request_desc, request_date FROM submiterequest_tb";
 ...
 if($result->num_rows > 0){
  while($row = $result->fetch_assoc()){
   ...
   echo '<form action="" method="POST"><input type="hidden" name="id" value='. $row["request_id"] .'><input type="submit" class="btn btn-danger mr-3" name="view" value="View">
...
if(isset($_REQUEST['close'])){
  $sql = "DELETE FROM submiterequest_tb WHERE request_id = {$_REQUEST['id']}";
